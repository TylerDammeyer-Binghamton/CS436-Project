...
class DecoderLayer(nn.Module):
    def __init__(self, self_attention, cross_attentions, d_model, d_ff=None, dropout=0.1, activation="relu"):
        super(DecoderLayer, self).__init__()
        d_ff = d_ff or 4 * d_model
        self.self_attention = self_attention
    ...
    def forward(self, x, multi_scale_cross, x_mask=None, cross_masks=None):
        x = x + self.dropout(self.self_attention(
            x, x, x,
            attn_mask=x_mask
        )[0])
...
class Decoder(nn.Module):
    ...
    def forward(self, x, multi_scale_cross, x_mask=None, cross_masks=None):
        for layer in self.layers:
            x = layer(x, multi_scale_cross, x_mask=x_mask, cross_masks=cross_masks)
